...
try:
    ...
    smart_cache = SmartMatrixCache(initial_size=1000, max_size=5000)
    print("✓ Advanced optimizations loaded: Vectorization + Caching enabled")
except ImportError:
    OPTIMIZATIONS_AVAILABLE = False
    cached_float_conversion = float  # Fallback to regular float
    print("⚠ Advanced optimizations not available, using standard implementation")
...
def noe2matrix(result,factors,flag_cchlist):
    ...
    for linei in HMQC:
       ...
       if noe_peaki_list:
         ...
         # OPTIMIZED: Vectorized peak neighbor analysis if available
         if OPTIMIZATIONS_AVAILABLE and len(noe_peaki_parsed) > 10:  # Use vectorization for larger datasets
             # Convert to NumPy arrays for vectorized operations
             noe_freqs = np.array([d['freq1'] for d in noe_peaki_parsed])
             hmqc_freqs = np.array([d['freq1'] for d in hmqc_parsed])
         ...
         for noe_peaki_data in noe_peaki_parsed:
             ...
             linei_freq1 = cached_float_conversion(linei_split[1]) if OPTIMIZATIONS_AVAILABLE else float(linei_split[1])
