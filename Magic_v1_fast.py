...
# Import lightweight caching optimizations
try:
    ...
    print("✓ Fast caching optimizations enabled")
except ImportError:
    CACHE_AVAILABLE = False
    cached_float_conversion = float
    print("⚠ Caching not available, using standard float")
...
# Use imported cached version if available, otherwise use local
if not CACHE_AVAILABLE:
    @lru_cache(maxsize=10000)
    def cached_float_conversion(value):
        """Cache float conversions for frequently converted values"""
        return cached_float_conversion(value)
...
def distances(pdb,Cutoff,lowCut,flag):
  ...
  for linei in Atomlist:
    spliti=linei.split()
    xi=cached_float_conversion(spliti[6])
    yi=cached_float_conversion(spliti[7])
    zi=cached_float_conversion(spliti[8])
    for aa in convert:
      if spliti[3]==aa[0]: Res_name=aa[1]
    if spliti[2]!='H': Methyl_list.append(Res_name+spliti[5]+spliti[2])  
    for linef in Atomlist:
      splitf=linef.split()
      xf=cached_float_conversion(splitf[6])
      yf=cached_float_conversion(splitf[7])
      zf=cached_float_conversion(splitf[8])
      if (xf!=xi or yf!=yi or zf!=zi):
        ...
        if (spliti[2]!='H' and splitf[2]!='H'): # CCH
            ...
            elif d>lowCut2 and d<=Cutoff2:coef=(Cutoff2-d)/cached_float_conversion(Cutoff2-lowCut2)
  ...
  for line in distances_CHCH:
    ...
    #if (flag=='run' and cached_float_conversion(line[2])!=0):methyl_file.write(str(distances_CHCH_new[-1])+'\n')
  Methyl_list_new=[]
...
def Extract_3DPeaks(object, peak_list_3d, peak_list_2d_CH, nuclei, tolerance):
  ...
  for line_noe in peak_list_3d:
    ...
    tot_intensity+=cached_float_conversion(split_noe[4])
    liste_NOE.append(cached_float_conversion(split_noe[4]))
    if cached_float_conversion(split_noe[4])>Max_noe:Max_noe=cached_float_conversion(split_noe[4])
  mean_intensity_per_strip=tot_intensity/cached_float_conversion(len(peak_list_2d_CH))
  mean_intensity=tot_intensity/cached_float_conversion(len(peak_list_3d))
  ...
  file_log.write('NOEs per strip: '+str(round(len(peak_list_3d)/cached_float_conversion(len(peak_list_2d_CH)),2))+'\n')
  file_log.close()
  for line_hmqc in peak_list_2d_ref:
      split_hmqc=line_hmqc.split()
      w=cached_float_conversion(split_hmqc[1])
      wh=cached_float_conversion(split_hmqc[2])
 ...
 #     elif nuclei[0]=='1H': wnoe=wh
      for line_noe in peak_list_3d:
        split_noe=line_noe.split()	
        w1=cached_float_conversion(split_noe[1])	
        w2=cached_float_conversion(split_noe[2])
        w3=cached_float_conversion(split_noe[3])
        if (w2>(w-w2tol) and 
            w2<(w+w2tol) and 
            w3>(wh-w3tol) and 
            w3<(wh+w3tol) and 
...
def matrix_it(links, element_list, flag, factors,geminal_mark):
  ...
  for line in links:
    i=element_index[line[0]]  # O(1) lookup instead of O(N) index()
    j=element_index[line[1]]  # O(1) lookup instead of O(N) index()
    if flag=='peak':matrix[i,i]=1    
    if flag=='pdb':
      matrix[i,j]=line[2]
      if i==j:matrix_geminal[i,j]=1
    if flag=='peak':
      chi=cached_float_conversion(line[6])
      if ((cached_float_conversion(line[2])>0 and line[7]==1) or factors[2]>0):
        if (line[4]==0):matrix[i,j]=math.pow(math.pow(1+cached_float_conversion(line[5]),2)*cached_float_conversion(line[7])*cached_float_conversion(factors[0]/(20*chi*math.pow(float(line[3]),2))),0.5)
        elif (line[4]==1):matrix[i,j]=math.pow(math.pow(1+cached_float_conversion(line[5]),2)*cached_float_conversion(line[7])*cached_float_conversion(factors[1]/(20*chi*math.pow(float(line[3]),2))),0.5)
        elif (line[4]==2):matrix[i,j]=math.pow(math.pow(1+cached_float_conversion(line[5]),2)*cached_float_conversion(line[7])*cached_float_conversion(factors[2]/(20*chi*math.pow(float(line[3]),2))),0.5)
      else:matrix[i,j]=0
  ...
  if flag=='peak':
    ...
    for line in links:
      ...
      chi=cached_float_conversion(line[6])
      matrix_ri[i,j]=cached_float_conversion(line[2])
      matrix_scoring[i,j]=cached_float_conversion(line[7])*math.pow(math.pow(1+cached_float_conversion(line[5]),2)/(20*chi*math.pow(cached_float_conversion(line[3]),2)),0.5)
...
def noe2matrix(result,factors,flag_cchlist):
    ...
    for i in range(len(HMQC)):
      ...
      for j in range(len(HMQC)):
        ...
        if (cached_float_conversion(peak_around_split[1])>(cached_float_conversion(checked_peak_split[1])-0.1) and 
            cached_float_conversion(peak_around_split[1])<(cached_float_conversion(checked_peak_split[1])+0.1) and
            cached_float_conversion(peak_around_split[2])>(cached_float_conversion(checked_peak_split[2])-0.01) and 
            cached_float_conversion(peak_around_split[2])<(cached_float_conversion(checked_peak_split[2])+0.01) and
    ...
    for linei in HMQC:
       ...
       try:
           ...
           for peak in noe_peaki_list:
             ...
             total_height_i+=cached_float_conversion(peak_split[4])
             if cached_float_conversion(peak_split[4])>imax:imax=cached_float_conversion(peak_split[4])
           if total_height_i>0:rimax=cached_float_conversion(imax/total_height_i)
           height_i2mean=round(cached_float_conversion(total_height_i)/DistNOE[0], 2)
       except AttributeError: pass
       if not str(linei_split[0]) in biblio_crosspeaks:biblio_crosspeaks[str(linei_split[0])]={}     
       if noe_peaki_list:
         counter=0     
         for noe_peaki in noe_peaki_list:
             noe_peaki_split=noe_peaki.split()
             ri=round(cached_float_conversion(noe_peaki_split[4])/total_height_i, 2)
             ri2max=round(cached_float_conversion(noe_peaki_split[4])/DistNOE[1],3)    
             peak_neighbors_clustering=[]
             peak_neighbors_scoring=[]
             shared_peak_neighbors=0
             for linef in HMQC:
                 linef_split=linef.split()
                 if ((cached_float_conversion(noe_peaki_split[1])>(cached_float_conversion(linef_split[1])-ppm_range[0])) and 
                     (cached_float_conversion(noe_peaki_split[1])<(cached_float_conversion(linef_split[1])+ppm_range[0]))):
                   shared_peak_neighbors=0
                   sym=sym_coef
                   ppm_matching=[0.035,0.035]
                   ppm_matching[0]=cached_float_conversion(noe_peaki_split[1])-cached_float_conversion(linef_split[1])
                   try:
                   ...
                   if noe_peakf_list:
                     for noe_peakf in noe_peakf_list:
                         ...
                         for noe_peakii in noe_peaki_list:
                           ...
                           if ((cached_float_conversion(noe_peakii_split[1])>(cached_float_conversion(noe_peakf_split[1])-ppm_range[0])) and 
                               (cached_float_conversion(noe_peakii_split[1])<(cached_float_conversion(noe_peakf_split[1])+ppm_range[0]))
                                ):shared_peak_neighbors+=1
                         if ((cached_float_conversion(noe_peakf_split[1])>(cached_float_conversion(linei_split[1])-ppm_range[0])) and 
                             (cached_float_conversion(noe_peakf_split[1])<(cached_float_conversion(linei_split[1])+ppm_range[0])) and
                             (not str(linef_split[0]) in peak_neighbors_clustering)):
                             ppm_matching[1]=cached_float_conversion(noe_peakf_split[1])-cached_float_conversion(linei_split[1])
                             sym=1
             ...
             for i in range(len(peak_neighbors_clustering)):
               ...
               d1=cached_float_conversion(peak_neighbors_clustering[i][2][0])
               d2=cached_float_conversion(peak_neighbors_clustering[i][2][1])
               ...
               if flag_cchlist==0:
                 score=round(math.pow(math.pow(1+cached_float_conversion(peak_neighbors_clustering[i][1]),2)*cached_float_conversion(sym)*cached_float_conversion(factors[0]/(20*chi*math.pow(float(N),2))),0.5),3)
             ...
             for i in range(len(peak_neighbors_scoring)):
               ...
               if (a==1 or n>=2):height=0
                 ...
                 a,n=0,1
                 height=cached_float_conversion(noe_peaki_split[4])
                 if flag_cchlist==2:height=cached_float_conversion(ri/rimax)
...
def build_assignment(index):
              ...
              report_follow.close()              
              file_log=open('./'+str(Time_start).split('.')[0].split('.')[0]+'/log', 'a')
              file_size=int(1000+cached_float_conversion(file_size_max*size/float(5+size)))              
              file_log.write('file_size: '+str(file_size)+'\n')
              file_log.close()
              for iii in index:             
                free_RAM=round(cached_float_conversion(psutil.virtual_memory().available)/cached_float_conversion(1024*1024*1024),1)              
                file_log=open('./'+str(Time_start).split('.')[0].split('.')[0]+'/log', 'a')
                ...
                for ii in range(len(assignment_archive_FINAL[1:])):
                  for element in possible_peak_assignments:
                    ...
                    if (flag_geminal==1 and matrix_noe_geminal.sum()>0):
                      ...
                      tot=round(testing.sum()*(1+0.2*(testing_geminal.sum()/cached_float_conversion(matrix_noe_geminal.sum()))),3)
                    #####Geminal scaling factor
          ...
          for assignment in total_Assignments[1:]:          
            ...
            try:
              ...
              for i in range(len(index_matrix_noe)):
                ...
                if peak in archive_assignment_result:
                  if not methyl in archive_assignment_result[peak]:
                    flag_stop=1
                    break
                  else:pass
                else:pass
            ...
            if (flag_geminal==1 and matrix_noe_geminal.sum()>0):
              ...
              testing_geminal=matrix_noe_geminal*matrix_assignment_geminal
              tot=round(tot*(1+0.2*(testing_geminal.sum()/cached_float_conversion(matrix_noe_geminal.sum()))),3)
            #####Geminal scaling factor
            else:pass
...
############## TO IMPROVE ###############
    ...
    for peak_name in list_peak:
      ...
      score.sort()
      score=score[::-1]
      score_max=cached_float_conversion(score[0][0])
      flag=0
      for i in range(len(score)):
        if i==0:continue
        elif cached_float_conversion(score[i][0])==score_max and i>0:
          flag=1
          break
        else:break        
...
############## TO IMPROVE ###############    
    ...
    for i in range(len(peaks)):
          ...
          try:
            if RESIDUE_PATTERN.search(str(methyl)).group(1)==PEAK_RESIDUE_PATTERN.search( peak).group(1):correct+=1
          except AttributeError:pass
          total+=1
          ...
          for j in range(matrix_score[i,:].size):
                if matrix_score[i,j]>0:
                  ...
                  cch_result_list.write('  '+str(round(matrix_peaks_CS[i,j],3))+'  d='+str(matrix_methyls[i,j])
                                        +'  '+str(a)+'\n')               
                  for NOE_index in biblio_crosspeaks[peak]:
                  ...
                  for NOE_index in biblio_crosspeaks[peak]:
                    if (peakNOE in biblio_crosspeaks[peak][NOE_index][0] and
                      ...
                      break
          if completeness.size>0:NOE_completeness=round(completeness.sum()/cached_float_conversion(completeness.size),2)
    ...
    for line in noelist[2:]:
            split=line.split()
            if cached_float_conversion(split[6])==0:continue
            else:pass
            ...
            sm=cached_float_conversion(split[4])
            for line_2 in noelist[2:]:
              ...
              if (res1f==res2i and name1f==name2i and res1i==res2f and name1i==name2f):
                sf=cached_float_conversion(split[4])
                sm=(sm+sf)/cached_float_conversion(2)
                break	
            M4=cached_float_conversion(M)
            M3=cached_float_conversion(3*M/float(4))
            M2=cached_float_conversion(2*M/float(4))
            M1=cached_float_conversion(M/float(4))
    ...
    for line in hmqclist[2:]:
        ...
        if split[3][0]!='N':
            ...
            name1i=re.search(r'[0-9]+(\w+)', split0[0]).group(1)
            try:
              score=cached_float_conversion(split[4])
              M4=cached_float_conversion(M)
              M3=cached_float_conversion(3*M/float(4))
              M2=cached_float_conversion(2*M/float(4))
              M1=cached_float_conversion(M/float(4))
...
flag_geminal=cached_float_conversion(start_file[13][:-1])
ppm_range=(cached_float_conversion(start_file[15].split()[0]),cached_float_conversion(start_file[15].split()[1]),cached_float_conversion(start_file[15].split()[2]))
cutoff_factor=cached_float_conversion(start_file[17][:-1])
lowCut,max_distance=cached_float_conversion(start_file[19].split()[0]),cached_float_conversion(start_file[19].split()[1])
...
cpu=mp.cpu_count()
Tot_RAM=round(cached_float_conversion(psutil.virtual_memory().total)/cached_float_conversion(1024*1024*1024),1)
free_RAM_start=round(cached_float_conversion(psutil.virtual_memory().available)/cached_float_conversion(1024*1024*1024),1)
...
file_size_max=int(1000000*(0.5*free_RAM_start-0.2*free_RAM_start)/cached_float_conversion(2*cpu*0.06))
file_size=10000
...
#if len(HMQC_peak_list)<=100:scaler=cached_float_conversion(1)
...
for line in SEQ:
  ...
  for methyl in metrics[2]:
    methylID=RESIDUE_PATTERN.search( methyl).group(1)
    if methylID==resID:
      flag=1
      break
  if flag==0:
    ...
    if P>0:
      ...
      for cluster_index in sort:
        if P>0:
            ...
            for line in assignment_archive_clusterX:
              ...
              for i in line_to_add:
                ...
                for peak in archive_assignment_result:
                  if methyl in archive_assignment_result[peak]:
                    ...
                    if (len(list(archive_assignment_result[peak].keys()))==1 and methyl[0]!='L' and methyl[0]!='V'):
                      ...
                      for peak2 in archive_assignment_result:
            ...
            ratio=round(cached_float_conversion((len(List_of_assigned_peaks)+len(involved_peak)+len(assignment_locked_peak))/cached_float_conversion(len(list_peak))),2)
        ...
        for element in pool_result:
              archive=element[0]
              for peak in archive:
                if not peak in archive_assignment_result:archive_assignment_result[peak]={}
                for methyl in archive[peak]:
                  if not methyl in archive_assignment_result[peak]:
                    archive_assignment_result[peak][methyl]=archive[peak][methyl]
                  if (methyl in archive_assignment_result[peak] and
        ...
        for file_name in list_of_files:
              peak_id=PEAK_ID_PATTERN.search( file_name).group(1)
        ...
        for peak in archive_assignment_result:
          for methyl in archive_assignment_result[peak]:
            try:
              if RESIDUE_PATTERN.search(str(methyl)).group(1)==PEAK_RESIDUE_PATTERN.search( peak).group(1):
        ...
        if flag_not_null==1:
          ...
          print('Tc= '+str(P)+'; '+str(Time_end-Time_start).split('.')[0].split('.')[0],' => ',str(round(100*len(list_of_peaks)/cached_float_conversion(len(HMQC_peak_list)),1)),'%')                 
...
while cycler==0:
    ...
    for index in sort:
      ...
      sum_local=peak_matrix_Scoring[involved_peak,:][:,List_of_assigned_peaks].sum()
      sum_global=peak_matrix_Scoring[List_of_assigned_peaks,:][:,involved_peak].sum()
      ratio=round((len(List_of_assigned_peaks)+len(assignment_locked_peak)+1)/cached_float_conversion(len(list_peak)),2)
      ...
      if flag_not_null==1:
        ...
        for peak in archive_assignment_result:
          if peak in archive_assignment_result_2save:
...
for peakline in file.readlines()[2:]:
  if peakline.count('NotAss')==0:
    ...
    if not str(number_of_methyls) in histo_ambiguity:
...
tot_ass=0
tot_met=0
sorted_histo_keys=[int(i) for i in histo_ambiguity]
sorted_histo_keys.sort()
file_log=open('./'+str(Time_start).split('.')[0]+'/log', 'a')
for key in sorted_histo_keys:
  val=histo_ambiguity[str(key)]
  file_log.write(str(key)+': '+str(val)+'\n')
  tot_ass+=cached_float_conversion(key)*cached_float_conversion(val)
  tot_met+=cached_float_conversion(val)
Time_end=time.datetime.now()
